...
pub const Lexer = struct {
    ...
    pub fn init(allocator: Allocator, input: []const u8, pointer: usize, char: u8) !Lexer {
        ...
        for (token.token_names) |tn| {
            try all_token_names.append(tn);
        }
        return .{
            .allocator = allocator,
            .cursor = cursor.Cursor.init(input, pointer, char),
    ...
    pub fn glob(self: *Lexer) !token.Token {
        const text = try fmt.allocPrint(self.allocator, "{c}", .{self.cursor.current_char});
        self.cursor.consume();
        return token.Token{
            .allocator = self.allocator,
            .kind = .glob,
...
test "expect Lexer initialization" {
    ...
    try testing.expectEqualDeep(token.token_names[0..], lexer.token_names.items);
...
test "expect Lexer to consume alias tokens" {
    ...
    try testing.expectEqual(token.TokenKind.alias, actual.kind);
}
...
test "expect Lexer to consume path tokens" {
    ...
    try testing.expectEqual(token.TokenKind.path, actual.kind);
}
